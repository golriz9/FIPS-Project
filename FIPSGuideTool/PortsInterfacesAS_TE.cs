...
{
	public partial class PortsInterfacesAS_TE : Form
	{
		...
		private void PortsInterfacesAS_TE_Load(object sender, EventArgs e)
		{
			// add columns
			...
			try
			{
				...
				while (dataSearch.Read())
				{
					if ((Section[ij] == "2") && (VendorTesterAll[ij] == "AS" || (VendorTesterAll[ij] == "TE")))
					{
						if ((PortsInterfacesSecurityLevel == "Level 1" || PortsInterfacesSecurityLevel == "Level 2") && ((RequirementAll[ij] == "16") || (RequirementAll[ij] == "17") || (RequirementAll[ij] == "18")))
						{
				...
				{
					if (Section[i1] == "2" && (VendorTesterAll[i1] == "AS" || (VendorTesterAll[i1] == "TE")))
					{
						text1[ij2] = text[i1];
						ij2++;
					}
		...
		private void PortsInterfacesAS_TE_FormClosing(object sender, FormClosingEventArgs e)
		{
			int ChangeFlag = 1;
			...
			if (ChangeFlag == 1)
			{
				...
				if (result == DialogResult.Yes)
				{
					...
					{
						if (VendorTesterAll[i1] == "AS" && Section[i1] == "2")
						{
						...
						if (VendorTesterAll[i1] == "TE" && Section[i1] == "2")
						{
							string inputStr = ModAssessment[i4];
							// Check if the string contains a single quote
							if (inputStr != null)
					...
					color_PortInter = SectionProgress(ij3);
					Properties.Settings.Default.color_PortInter = color_PortInter;
		...
		private void dataGridView_ASTE_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
		{
			connection.Open();
			command.Connection = connection;
		...
		public string SectionProgress(int[] ij)
		{
			...
			for (int i11 = 0; i11 < dataGridView_ASTE.Rows.Count - 1; i11++)
			{
				if (ij[i11] == 0)
				{
					...
					{
						color_PortInter = "False";
					}
				}
			}
			return color_PortInter;
		}		   		 	  	  	   	
