...
{
	public partial class KeyManagementAS_TE : Form
	{
		...
		private void KeyManagementAS_TE_Load(object sender, EventArgs e)
		{
			// add columns
			...
			try
			{
				...
				while (dataSearch.Read())
				{
					if ((Section[ij] == "7") && (VendorTesterAll[ij] == "AS" || (VendorTesterAll[ij] == "TE")))
					{
				...
				{
					if (Section[i1] == "7" && (VendorTesterAll[i1] == "AS" || (VendorTesterAll[i1] == "TE")))
					{
						text1[ij2] = text[i1];
						ij2++;
					}
		...
		private void KeyManagementAS_TE_FormClosing(object sender, FormClosingEventArgs e)
		{
			int ChangeFlag = 1;
			...
			if (ChangeFlag == 1)
			{
				...
				if (result == DialogResult.Yes)
				{
					...
					{
						if (VendorTesterAll[i1] == "AS" && Section[i1] == "7")
						{
						...
						if (VendorTesterAll[i1] == "TE" && Section[i1] == "7")
						{
							string inputStr = ModAssessment[i4];
							// Check if the string contains a single quote
							if (inputStr != null)
					...
					color_KeyManag = SectionProgress();
					Properties.Settings.Default.color_KeyManag = color_KeyManag;
